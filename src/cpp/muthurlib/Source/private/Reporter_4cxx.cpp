...
//#include <../activemq/apache-log4cxx-0.10.0/src/main/include/log4cxx/xml/domconfigurator.h>
...
void CReporter::OnDebug(EReporterLevel eLevel, char* pszModule, char* pszMethod, char* pszFormat, va_list paArgs)
{
	...
	//{
		/*if(m_logger != NULL)
		{
			LOG4CXX_DEBUG(m_logger, szMessage);
		}
		else
		{
			std::cout << " 1 DEBUG m_logger is null!" << std::endl;
		}
...
void CReporter::SetModuleName(char* pszModuleName)
{
	...
	// Initialize a logger for the given module (i.e. each module will get its own logger)
	//m_logger = log4cxx::Logger::getLogger(m_strModuleName);
