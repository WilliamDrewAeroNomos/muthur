 ...
 * @version $Revision$
 */
public class StartFederationRequestProcessorWithAutoStartFalseAndNoStartCallTest
	...
	public void runAllTests() {
		...
		baseFederationRequestProcessorWithAutoStartFalseAndNoStartCall();
	 ...
	 * 
	 */
	public void baseFederationRequestProcessorWithAutoStartFalseAndNoStartCall() {
		...
		// call start() on the federation execution controller which will release
		// the semaphore that the start federation request processor is blocking on.
		//

		sem = new Semaphore(0);

		// do not call start on the federation and test that the timer events are
		// not being sent. Without the timer events being sent we will time out
		// waiting for the semaphore count to be increased

		try {
			if (sem.tryAcquire(10, 30000, TimeUnit.MILLISECONDS)) {
				fail("Should have timed out before acquiring semaphore");
			} else {
				// this is where you want to be since you didn't call start() on the
				// federation
			}
		} catch (InterruptedException e) {
			fail(e.getLocalizedMessage());
